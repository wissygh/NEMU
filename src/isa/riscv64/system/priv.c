...
inline word_t gen_status_sd(word_t status) {
  mstatus_t mstatus = (mstatus_t)status;
  bool fs_dirty = MUXNDEF(CONFIG_FPU_NONE, mstatus.fs == EXT_CONTEXT_DIRTY, false);
  bool vs_dirty = MUXDEF(CONFIG_RVV, mstatus.vs == EXT_CONTEXT_DIRTY, false);
...
// If sstateen check after csr_is_legal, the exception type is wrong.
